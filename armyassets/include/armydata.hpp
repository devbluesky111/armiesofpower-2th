...
namespace armydata {
    ...
    typedef std::variant <\
        ...
        armydata::INT8_VEC, armydata::INT16_VEC, armydata::INT32_VEC, armydata::INT64_VEC, \
        armydata::UINT8_VEC, armydata::UINT16_VEC, armydata::UINT32_VEC, armydata::UINT64_VEC, \
        armydata::FLOAT_VEC, armydata::DOUBLE_VEC, armydata::STRING_VEC> ARMY_ATTRIBUTE;

    typedef std::map <std::string, ARMY_ATTRIBUTE> ATTRIBUTE_MAP;
    ...
    vector <uint8_t> serialize_attribute(const string &type, const ARMY_ATTRIBUTE &attr) {
        if (type.find("[]", type.length() - 2) == type.length() - 2) {
            ...
            if (std::holds_alternative <INT8_VEC>(attr)) {
                ...
                for (auto child : vec) {
                    ARMY_ATTRIBUTE child_attr = child;
    ...
    ARMY_ATTRIBUTE deserialize_attribute(const string &type, vector <const uint8_t>::iterator &itr) {
        if (type.find("[]", type.length() - 2) == type.length() - 2) {
            //Type is an array
